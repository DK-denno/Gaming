	...
	var layoutGeneration = function(layoutItems) {
		...
		**/
		...
		var defaultSSOElements = {
			desktop: ['desktopStart', 'masthead', 'siteLinks', 'support',  'accountsNew', 'loginRegisterButtons', 'locale', 'desktopEnd'],
			mobile: ['mobileStart', 'mobileButton', 'masthead', 'mobileLoginRegister', 'mobileEnd', 'modalStart', 'locale',  'siteLinks', 'support', 'accountsNew', 'modalEnd']
		};
	...
	var showAccounts = function() {
		...
		if (user.identities) {
			...
			if(!pgacctExists || pgacct == "") {
				...
				$.getJSON(SSO.utility.getApiUrl() + '/refreshToken?callback=?');
			}
