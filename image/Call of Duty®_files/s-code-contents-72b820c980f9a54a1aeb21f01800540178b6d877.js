...
function getAnalyticsAccount() {
    var s_account;
    if(_satellite.settings.isStaging==true){s_account="activision.stg"}else{s_account="activision.prd"}
    return s_account;
}
...
/************** MARKETING CLOUD ID SERVICE CONFIG  **************/
s.visitor=Visitor.getInstance("0FB367C2524450B90A490D4C@AdobeOrg")
...
s.linkInternalFilters="javascript:,activision.com,callofduty.com,skylanders.com,destinythegame.com,activision.com,guitarhero.com,activisionblizzard.com,activisionsupport.force.com,activisionsupport.secure.force.com,ambassador.force.com,ambassador.secure.force.com,playersupport.my.salesforce.com,na25.salesforce.com,treyarch.com,sledgehammergames.com,activisionsupport.force.com,highmoon.com,infinityward.com,ravensoftware.com,toysforbob.com,crashbandicoot.com" 
...
var dfaConfig = {
	...
	SPOTID:             '4807637', // DFA Spotlight ID
	tEvar:              'eVar33', // Transfer variable, typically the "View Through" eVar.
	errorEvar:          'eVar36', // DFA error tracking (optional)
...
function s_doPlugins(s) {
    ...
    //get adobe visitor id
    //s.visitor&&s.visitor.a.MCID?s.eVar30=s.visitor.a.MCMID:s.eVar30="VisitorAPI Missing";
    s.visitor&&s.visitor.getMarketingCloudVisitorID()?s.eVar30=s.visitor.getMarketingCloudVisitorID():s.eVar30="VisitorAPI Missing";
    
    //copy document.title
    s.eVar4=document.title;
    s.prop4=s.eVar4?"D=v4":"";
    
    //copy page URL
    s.eVar5=location.href;
    s.prop5=s.eVar5?"D=v5":"";

    //get previous page name
    s.prop8=s.getPreviousValue(s.pageName,'gpv_c8','');
    s.eVar8=s.prop8?"D=c8":"";

    //get percent of page viewed
    var ppv = s.getPercentPageViewed(s.pageName);
    if( ppv && typeof ppv=='object' && ppv[0] == s.prop8 ) {
        s.prop9=ppv[1] + '|' + ppv[2]; //prop9: prev page max and initial % viewed, delimited by "|"
        s.eVar9="D=c9";
    }
    
    //time parting
    s.prop10=s.getTimeParting('n','-8');
    s.eVar10=s.prop10?"D=c10":"";
    
    //new vs repeat visitor
    s.prop16= s.getNewRepeat();
    s.eVar16=s.prop16?"D=c16":"";
    
    //internal search and null search results
    if(s.prop12=="zero"){
        s.eVar14="D=v11";
        s.prop14="D=v11";
    ...
    //capture internal campaign once per visit
    s.eVar27=s.getValOnce(s.eVar27,'s_eVar27',0);

    //capture external campaign once per month
    s.campaign = s.Util.getQueryParam("cid");
    if (s.campaign)
        s.campaign=s.getValOnce(s.campaign,'s_campaign',30);
    
    //copy vars with dynamic var
    s.eVar1=s.prop1?"D=c1":"";
    s.eVar2=s.prop2?"D=c2":"";
    s.eVar3=s.prop3?"D=c3":"";
    s.eVar6=s.prop6?"D=c6":"";
    s.eVar7=s.prop7?"D=c7":"";
    s.eVar11=s.prop11?"D=c11":"";
    s.eVar12=s.prop12?"D=c12":"";
    s.eVar13=s.prop13?"D=c13":"";
    s.eVar17=s.prop17?"D=c17":"";
    s.eVar18=s.prop18?"D=c18":"";
    s.eVar19=s.prop19?"D=c19":"";
    s.eVar20=s.prop20?"D=c20":"";
    s.eVar21=s.prop21?"D=c21":"";
    s.eVar22=s.prop22?"D=c22":"";
    s.eVar23=s.prop23?"D=c23":"";
    s.eVar25=s.pageName?"D=pageName":"";
    s.eVar26=s.channel?"D=ch":"";    
}
...
s.trackingServer="activision.sc.omtrdc.net"
//s.trackingServerSecure="activision.ssl.sc.omtrdc.net"
...
+"erCase()+'=');gs=(scp)?q.indexOf('&'+scp.toLowerCase()+'='):-1;if(g>-1){s.vpr(p,cr);v=1;}else if(gs>-1){v=0;s.vpr(tv,'SearchCenterVisitors');}if(!s.c_w(c,cr,t)){s.c_w(c,cr,0);}if(!s.c_r(c)){v=0;}r"
+"eturn v>=1;"); 
...
if(ATVI && ATVI.components && ATVI.components.video) {
ATVI.library.onDependencyLoad("video", function() {
	ATVI.components.video.addYtEventHandler("stateChange", function(event) {
		...
		var mediaOffset = Math.floor(event.target.getCurrentTime() || 0);
...
if(s.enableVideoTracking){
    ...
    s.Media.trackVars='events,eVar15,eVar24,prop15';
    s.Media.trackEvents='event17,event18,event19,event20,event21,event22,event23';
}
/****************************** MODULES *****************************/
/*
 ActivityMap Module*/
